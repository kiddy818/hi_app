...
ceanic_log4cpp::ceanic_log4cpp(const char* name,int mode,const char* file_path,unsigned int max_file_len)
:m_category(log4cpp::Category::getRoot().getInstance(std::string(name)))
{
	if(mode & CEANIC_LOG_MODE_CONSOLE)
	{
...
ceanic_log4cpp::~ceanic_log4cpp()
{

}

ceanic_log4cpp* ceanic_log4cpp::create_log(const char* name,int mode,const char* file_path,unsigned int max_file_len /* = 0 */)
{
	return new ceanic_log4cpp(name,mode,file_path,max_file_len);
}

bool ceanic_log4cpp::set_level(CEANIC_LOG_LEVEL_E level)
{
...
bool ceanic_log4cpp::info(const char* msg)
{
	m_category.info(msg);
...
bool ceanic_log4cpp::warn(const char* msg)
{
    m_category.warn(msg);
...
bool ceanic_log4cpp::debug(const char* msg)
{
...
bool ceanic_log4cpp::error(const char* msg)
{	
...
bool ceanic_log4cpp::fatal(const char* msg)
{	
