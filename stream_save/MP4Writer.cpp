...
s32 MP4Writer::Init265(u32 TimeScale)
{
    ...
    if (0 == m_265TrackIndex)
        return -1;
    if (GF_OK != gf_isom_set_track_enabled(m_ptFile, m_265TrackIndex, GF_TRUE))
        return -1;
    ...
    while (1)
    {
        ...
        if (CONFIG_FINISH == m_Video265Statue)
        {
            ...
            *((u32 *)pData) = htonl(NaluSize - 4); /* 这里的长度不能包括前四个字节的头！ */
            tISOSample.data = (unsigned char *)pData;
        ...
        else if (CONFIG_STATUS == m_Video265Statue)
        {
            ...
            {
                ID = gf_media_hevc_read_vps((unsigned char *)pData , NaluSize, ptHEVCState);
                ...
                m_tAVCConfig_VPS.data = (unsigned char *)gf_malloc(NaluSize);
            ...
            {
                ID = gf_media_hevc_read_sps((unsigned char *)pData, NaluSize, ptHEVCState);
                ...
                m_tAVCConfig_SPS.data = (unsigned char *)gf_malloc(NaluSize);
            ...
            else if (34 == NaluType && NULL == m_tAVCConfig_PPS.data) /* PPS */
            {
                ...
                m_tAVCConfig_PPS.data = (unsigned char *)gf_malloc(NaluSize);
...
s32 MP4Writer::InitAAC(u8 AudioType, u32 SampleRate, u8 Channel, u32 TimeScale)
{
    ...
    if (GF_OK != gf_isom_set_track_enabled(m_ptFile, m_AACTrackIndex, GF_TRUE))
        return -1;
    ...
    ptStreamDesc->decoderConfig->objectTypeIndication = GF_CODECID_AAC_MPEG4;
    ptStreamDesc->decoderConfig->decoderSpecificInfo->dataLength = 2;
    ptStreamDesc->decoderConfig->decoderSpecificInfo->data = (unsigned char *)&AudioConfig;
    ...
    gf_isom_set_pl_indication(m_ptFile, GF_ISOM_PL_AUDIO, (GF_ISOProfileLevelType)AACProfile);
